 ...
 */
package org.thoughtcrime.securesms.bench;
 ...
 */
public class SignalServiceMessageSenderH {

    private static final int RETRY_COUNT = 4;
    SignalServiceMessageSender messageSender;

    public SignalServiceMessageSenderH(SignalServiceMessageSender messageSender)
    {
    ...
    public CallingResponse makeCallingRequest(long requestId, String url, String httpMethod, List<Pair<String, String>> headers, byte[] body) {
        return messageSender.socket.makeCallingRequest(requestId, url, httpMethod, headers, body);
    }
            ...
            throws UntrustedIdentityException, IOException
    {
        ...
        if (result.getSuccess() != null && result.getSuccess().isNeedsSync()) {
            ...
            sendMessage(messageSender.localAddress, Optional.<UnidentifiedAccess>absent(), timestamp, syncMessage, false, null);
        }

        // TODO [greyson][session] Delete this when we delete the button
        if (message.isEndSession()) {
            if (recipient.getUuid().isPresent()) {
                messageSender.store.deleteAllSessions(recipient.getUuid().get().toString());
            }
            if (recipient.getNumber().isPresent()) {
                messageSender.store.deleteAllSessions(recipient.getNumber().get());
            }

            if (messageSender.eventListener.isPresent()) {
                messageSender.eventListener.get().onSecurityEvent(recipient);
            }
        }
            ...
            throws IOException, UntrustedIdentityException
    {
        ...
        if (needsSyncInResults || messageSender.isMultiDevice.get()) {
            ...
            throws IOException, UntrustedIdentityException
    {
        ...
        sendMessage(messageSender.localAddress, Optional.<UnidentifiedAccess>absent(), timestamp, content, false, null);
    }

    public void setSoTimeoutMillis(long soTimeoutMillis) {
        messageSender.socket.setSoTimeoutMillis(soTimeoutMillis);
    }

    public void cancelInFlightRequests() {
        messageSender.socket.cancelInFlightRequests();
    }

    public void update(SignalServiceMessagePipe pipe, SignalServiceMessagePipe unidentifiedPipe, boolean isMultiDevice) {
        messageSender.pipe.set(Optional.fromNullable(pipe));
        messageSender.unidentifiedPipe.set(Optional.fromNullable(unidentifiedPipe));
        messageSender.isMultiDevice.set(isMultiDevice);
    }
            ...
            throws NonSuccessfulResponseCodeException, PushNetworkException, MalformedResponseException
    {
        ...
        Optional<SignalServiceMessagePipe> localPipe          = messageSender.pipe.get();
        ...
        if (v2UploadAttributes == null) {
            v2UploadAttributes = messageSender.socket.getAttachmentV2UploadAttributes();
        }

        Pair<Long, byte[]> attachmentIdAndDigest = messageSender.socket.uploadAttachment(attachmentData, v2UploadAttributes);
    ...
    public ResumableUploadSpec getResumableUploadSpec() throws IOException {
        ...
        if (v3UploadAttributes == null) {
            v3UploadAttributes = messageSender.socket.getAttachmentV3UploadAttributes();
        }

        return messageSender.socket.getResumableUploadSpec(v3UploadAttributes);
    }
    ...
    private SignalServiceAttachmentPointer uploadAttachmentV3(SignalServiceAttachmentStream attachment, byte[] attachmentKey, PushAttachmentData attachmentData) throws IOException {
        byte[] digest = messageSender.socket.uploadAttachment(attachmentData);
            ...
            throws IOException, UntrustedIdentityException
    {
        ...
        if (result.getSuccess().isNeedsSync()) {
            ...
            sendMessage(messageSender.localAddress, Optional.<UnidentifiedAccess>absent(), message.getTimestamp(), syncMessage, false, null);
        }
    }
            ...
            throws IOException
    {
        ...
        while (recipientIterator.hasNext()) {
            ...
            futureResults.add(messageSender.executor.submit(() -> sendMessage(recipient, access, timestamp, content, online, cancelationSignal)));
        }
            ...
            throws UntrustedIdentityException, IOException
    {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            try {
                OutgoingPushMessageList messages = getEncryptedMessages(messageSender.socket, recipient, unidentifiedAccess, timestamp, content, online);
                ...
                Optional<SignalServiceMessagePipe> pipe             = messageSender.pipe.get();
                Optional<SignalServiceMessagePipe> unidentifiedPipe = messageSender.unidentifiedPipe.get();
                ...
                if (pipe.isPresent() && !unidentifiedAccess.isPresent()) {
                    try {
                        ...
                        return SendMessageResult.success(recipient, false, response.getNeedsSync() || messageSender.isMultiDevice.get(), System.currentTimeMillis() - startTime);
                    ...
                    try {
                        ...
                        return SendMessageResult.success(recipient, true, response.getNeedsSync() || messageSender.isMultiDevice.get(), System.currentTimeMillis() - startTime);
                    } catch (IOException | ExecutionException | InterruptedException | TimeoutException e) {
                    }
                }
                ...
                SendMessageResponse response = messageSender.socket.sendMessage(messages, unidentifiedAccess);

                return SendMessageResult.success(recipient, unidentifiedAccess.isPresent(), response.getNeedsSync() || messageSender.isMultiDevice.get(), System.currentTimeMillis() - startTime);
                ...
                handleMismatchedDevices(messageSender.socket, recipient, mde.getMismatchedDevices());
            ...
            throws IOException, InvalidKeyException, UntrustedIdentityException
    {
        ...
        if (!recipient.matches(messageSender.localAddress) || unidentifiedAccess.isPresent()) {
            messages.add(getEncryptedMessage(socket, recipient, unidentifiedAccess, SignalServiceAddress.DEFAULT_DEVICE_ID, plaintext));
        }

        for (int deviceId : messageSender.store.getSubDeviceSessions(recipient.getIdentifier())) {
            if (messageSender.store.containsSession(new SignalProtocolAddress(recipient.getIdentifier(), deviceId))) {
            ...
            throws IOException, InvalidKeyException, UntrustedIdentityException
    {
        ...
        SignalServiceCipherH   cipher                = new SignalServiceCipherH(messageSender.localAddress, messageSender.store, messageSender.sessionLock, null);

        if (!messageSender.store.containsSession(signalProtocolAddress)) {
            try {
                ...
                for (PreKeyBundle preKey : preKeys) {
                    try {
                        ...
                        SignalSessionBuilder  sessionBuilder = new SignalSessionBuilder(messageSender.sessionLock, new SessionBuilder(messageSender.store, preKeyAddress));
            ...
            throws IOException, UntrustedIdentityException
    {
        try {
            for (int extraDeviceId : mismatchedDevices.getExtraDevices()) {
                if (recipient.getUuid().isPresent()) {
                    messageSender.store.archiveSession(new SignalProtocolAddress(recipient.getUuid().get().toString(), extraDeviceId));
                }
                if (recipient.getNumber().isPresent()) {
                    messageSender.store.archiveSession(new SignalProtocolAddress(recipient.getNumber().get(), extraDeviceId));
                }
            }
            ...
            for (int missingDeviceId : mismatchedDevices.getMissingDevices()) {
                ...
                try {
                    SignalSessionBuilder sessionBuilder = new SignalSessionBuilder(messageSender.sessionLock, new SessionBuilder(messageSender.store, new SignalProtocolAddress(recipient.getIdentifier(), missingDeviceId)));
                    sessionBuilder.process(preKey);
                } catch (org.whispersystems.libsignal.UntrustedIdentityException e) {
                    throw new UntrustedIdentityException("Untrusted identity key!", recipient.getIdentifier(), preKey.getIdentityKey());
                }
    ...
    private void handleStaleDevices(SignalServiceAddress recipient, StaleDevices staleDevices) {
        for (int staleDeviceId : staleDevices.getStaleDevices()) {
            if (recipient.getUuid().isPresent()) {
                messageSender.store.archiveSession(new SignalProtocolAddress(recipient.getUuid().get().toString(), staleDeviceId));
            }
            if (recipient.getNumber().isPresent()) {
                messageSender.store.archiveSession(new SignalProtocolAddress(recipient.getNumber().get(), staleDeviceId));
            }
        }
    }
    ...
    private byte[] enforceMaxContentSize(byte[] content) {
        if (messageSender.maxEnvelopeSize > 0 && content.length > messageSender.maxEnvelopeSize) {
